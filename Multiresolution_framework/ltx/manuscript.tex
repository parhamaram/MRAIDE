...
\end{equation}
where $v\left(\mathbf{r},t\right)$ is a voltage field that may be measured with a multi-electrode array or voltage sensitive dye, $\mathbf{r}\in \mathbb{R}^n$ (where $n\le3$) describes the spatial location with the field, $t$ denotes time, and $j$ indexes the different levels of resolution in the field. 
...
\end{equation}
where the post-synaptic membrane voltage at time $t$ of a population of neurons at position $\mathbf r$ is denoted $v_t\left(\mathbf r\right)$. Membrane dynamics are included through the parameter $\xi=1-T_s/\tau$, where $\tau$ is the membrane time constant and $T_s$ is the sampling time. The connectivity strength between neural populations at a distance $\mid\mathbf{r}-\mathbf{r'}\mid$ is described by the connectivity kernel $w\left(\mathbf{r}-\mathbf{r}'\right)$. The connectivity kernel is taken as a ``Mexican hat'' function, which describes local excitation and lateral inhibition \cite{Amari1977,Atay2005}. The term $e_t(\mathbf r)$ is a zero mean Gaussian disturbance, spatially coloured but temporally white, with covariance function 
\begin{equation}
...
The multi-resolution approximation \cite{Mallat1989a} of the neural IDE (MRAIDE) is obtained by decomposing both the field, $v_t(\cdot)$, and the connectivity kernel, $w(\cdot)$, (assuming square-integrable functions) using translations and dilations of a scaling function $\phi(\mathbf{r})$ and a mother wavelet $\psi(\mathbf{r})$. Considering an $n$-dimensional field, the static connectivity kernel is decomposed as,
\begin{equation}
 w\left(\mathbf{r}\right)=\sum_{\mathbf{l} \in \mathbb{Z}^n}\alpha_{j_0,\mathbf{l}} \underbrace{\phi_{j_0,\mathbf{l}}\left(\mathbf{r}\right)}_{\text{scaling}} + \sum_{j\geq j_0}^{\infty} \sum_{\mathbf{l} \in \mathbb{Z}^n}\beta_{j,\mathbf{l}} \underbrace{\psi_{j,\mathbf{l}}\left(\mathbf{r}\right)}_{\text{wavelet}}, 
\label{eq:KernelExpansion}
\end{equation}
where $\alpha_{j_0,\mathbf{l}}$ are the approximation coefficients at the lowest scale $j_0$, and $\beta_{j,\mathbf{l}}$ are the detail coefficients at different scales $j$, with
\begin{align}
\phi_{j,\mathbf{l}}\left(\mathbf{r}\right) =& 2^{\frac{j}{2}}\phi\left(2^j\mathbf{r}-\mathbf{l}\right) \\	
\psi_{j,\mathbf{l}}\left(\mathbf{r}\right) =& 2^{\frac{j}{2}}\psi\left(2^j\mathbf{r}-\mathbf{l}\right).
\end{align}
The parameters $j$ and $\mathbf{l}$ control the scale (dilation) and translation (spatial shift), respectively. \dean{NEED SOMETHING ABOUT HOW j AND l ARE SET HERE - AS IN THAT THEY ARE KNOWN - AND THAT WE ONLY NEED TO ESTIMATE THE COEFFICIENTS.} Scaling functions retain the lowest frequency components, due to their low-pass properties, while the wavelet functions extract successively higher frequency components due to their band-pass characteristics. 
...
\begin{equation}
 v_t\left(\mathbf{r}\right)=\sum_{\mathbf{l} \in \mathbb{Z}^n}x_{t,j_{0},\mathbf{l}} \underbrace{\phi_{j_0,\mathbf{l}}\left(\mathbf{r}\right)}_{\text{scaling}} + \sum_{j\geq j_0}^{\infty} \sum_{\mathbf{l} \in \mathbb{Z}^n} \check{x}_{t,j,\mathbf{l}} \underbrace{\psi_{j,\mathbf{l}}\left(\mathbf{r}\right)}_{\text{wavelet}},
\label{eq:FieldExpansion}
\end{equation}
where $x_{t,j_{0},\mathbf{l}}$ and $\check{x}_{t,j,\mathbf{l}} $ are the dynamic coefficients of the expansion, constituting the state vector at time $t$. \dean{The decomposition enables a separation of the static spatial characteristics, which are considered static functions, that are weighted by dynamic coefficients.  }
...
Now we will simplify the notation by defining vectors that contain all the translations of the wavelets and scaling functions such that
\begin{align}
	\boldsymbol\alpha_{k} \triangleq& \left\lbrace\alpha_{k, \mathbf{l}}:\mathbf{l} \in \mathbb{Z}^n \right\rbrace \\
	\boldsymbol\beta_{k} \triangleq& \\
	\mathbf{x}_{t,k} \triangleq& \\
	\check{\mathbf{x}}_{t,k} \triangleq& \\
	\boldsymbol\phi_{j_0}^\top(\mathbf{r}) \triangleq& \\
	\boldsymbol\psi_{k}^\top(\mathbf{r}) \triangleq& ,
\end{align}
...
\begin{align}
	w\left(\mathbf{r}\right) &\approx \boldsymbol\theta^\top\boldsymbol\lambda\left(\mathbf{r}\right) 
	\label{eq:KernelFiniteExpansion} \\
	v_t\left(\mathbf{r}\right) &\approx \boldsymbol\mu^\top\left(\mathbf{r}\right)\mathbf{x}_t.
...
\begin{equation}\label{eq:ApproxDiscreteTimeModel}
	\boldsymbol\mu^\top\left(\mathbf{r}\right)\mathbf{x}_{t+1} = 
	\xi \boldsymbol\mu^\top\left(\mathbf{r}\right)\mathbf{x}_t + 
	T_s \int_\Omega { 
	    \boldsymbol\theta^\top\boldsymbol\lambda\left(\mathbf{r}-\mathbf{r}'\right)
	    f\left(\boldsymbol\mu^\top\left(\mathbf{r}'\right)\mathbf{x}_t\right) 
	\, \mathrm{d}\mathbf{r}'}  
	+ e_t\left(\mathbf{r}\right).
\end{equation}
...
\begin{equation}\label{eq:ApproxDiscreteTimeModel2}
	\int_{\Omega} \boldsymbol\mu\left(\mathbf{r}\right)\boldsymbol\mu^\top\left(\mathbf{r}\right) \mathrm{d}\mathbf{r} \mathbf{x}_{t+1} = 
	\xi \int_{\Omega}\boldsymbol\mu\left(\mathbf{r}\right)\boldsymbol\mu^\top\left(\mathbf{r}\right) \mathrm{d}\mathbf{r} \mathbf{x}_t + 
	T_s \int_{\Omega}\boldsymbol\mu\left(\mathbf{r}\right)\int_\Omega { 
	    \boldsymbol\theta^\top\boldsymbol\lambda\left(\mathbf{r}-\mathbf{r}'\right)
	    f\left(\boldsymbol\mu^\top\left(\mathbf{r}'\right)\mathbf{x}_t\right) 
	\, \mathrm{d}\mathbf{r}'\mathrm{d}\mathbf{r}}  
	+ \int_{\Omega}\boldsymbol\mu\left(\mathbf{r}\right)e_t\left(\mathbf{r}\right)\mathrm{d}\mathbf{r}.
\end{equation}
To isolate the state vector, we now define the matrix 
\begin{equation}
	\label{eq:Lambdax}
	 \mathbf{\Lambda}_{x} \triangleq \int_{\Omega}\boldsymbol{\mu}\left(\mathbf{r}\right)\boldsymbol{\mu}^\top\left(\mathbf{r}\right) \mathrm{d}\mathbf{r}
\end{equation}
...
\begin{equation}\label{eq:ApproxDiscreteTimeModel4}
	\mathbf{x}_{t+1} = 
	\xi \mathbf{x}_t + 
	T_s \mathbf{\Lambda}_{x}^{-1} \int_{\Omega}\boldsymbol\mu\left(\mathbf{r}\right)\int_\Omega { 
	    \boldsymbol\theta^\top\boldsymbol\lambda\left(\mathbf{r}-\mathbf{r}'\right)
	    f\left(\boldsymbol\mu^\top\left(\mathbf{r}'\right)\mathbf{x}_t\right) 
	\, \mathrm{d}\mathbf{r}'\mathrm{d}\mathbf{r}}  
	+ \mathbf{\Lambda}_{x}^{-1}\int_{\Omega}\boldsymbol{\mu}\left(\mathbf{r}\right) e_t\left(\mathbf{r}\right) \mathrm{d}\mathbf{r}.
\end{equation}
...
\mathbf w_t= \mathbf{\Lambda}_{x}^{-1}\int_{\Omega}\boldsymbol\mu \left(\mathbf{r}\right)e_t\left(\mathbf{r}\right)\,\mathrm{d}\mathbf{r},
\end{equation}
with covariance (see \cite{Freestone2011} for proof)
\begin{equation}\label{eq:CovMatrix}
\boldsymbol\Sigma_w =\mathbf{\Lambda}_{x}^{-1}\iint\limits_{\boldsymbol\Omega}\boldsymbol\mu\left(\mathbf{r}\right) \eta\left(\mathbf{r}-\mathbf{r}'\right)\boldsymbol\mu^{\top}\left(\mathbf{r}'\right)\,\mathrm{d}\mathbf{r}'\mathrm{d}\mathbf{r}\mathbf{\Lambda}_{x}^{-\top}.
\end{equation} 

The state-transition equation can be simplified by exploiting the isotropy of the scaling functions and the wavelets where
\begin{equation}
	\boldsymbol{\lambda}_{j,l}(\mathbf{r}-\mathbf{r}') = \boldsymbol{\lambda}(2\mathbf{c}_{j,l}+\mathbf{r}'-\mathbf{r}). 
\end{equation}
Following this, we can switch the order of operations with the inner product and convolution in equation~\eqref{eq:ApproxDiscreteTimeModel4}. With this switch the convolution is now between the scaling functions and wavelets for the field and the scaling functions and wavelets for the connectivity kernel. This convolution can be calculated analytically when using B-spline wavelets and can be predefined once, since these are static functions. This has major computation advantages when solving the estimation problem. The predefined convolution can be written as
\begin{equation}
	\left[\Gamma(\mathbf{r}')\right]_{:j,l} = T_s \mathbf{\Lambda}_{x}^{-1}\int_{\Omega} \boldsymbol\mu\left(\mathbf{r}\right)\boldsymbol\lambda\left(2\mathbf{c}_{j,l} + \mathbf{r}'-\mathbf{r}\right) \,\mathrm{d}\mathbf{r}
\end{equation}
giving
\begin{equation}
	\mathbf{x}_{t+1} = 
	\xi \mathbf{x}_t + 
	\int_{\Omega} \Gamma\left(\mathbf{r}'\right)f\left(\boldsymbol\mu^\top\left(\mathbf{r}'\right) \mathbf{x}_t\right) 
	\, \mathrm{d}\mathbf{r}' \boldsymbol\theta
	+ \mathbf w_t
\end{equation}
...
\begin{equation}\label{eq:Observationmatrix}
	\mathbf{C}_{ik} \triangleq \int_{\Omega}m(\mathbf{r}_i - \mathbf{r}')\boldsymbol{\mu}_k(\mathbf{r}') \, \mathrm{d}\mathbf{r}'.
\end{equation}
...
\begin{equation}\label{eq:MRA-DoBoorFormula}
 ...
 \end{equation}
The most commonly used form of splines is the cubic spline $\left(m=4\right)$, comprised of third order polynomials added together at the joining points, with an explicit expression given by
\begin{align}
N_{4}\left(r\right)=
\begin{cases}
...
The multi-resolution approximation using B-spline functions is completed by defining a two-scale relation pair, which for cardinal B-spline scaling and wavelet functions of order $m$ takes the form of
\begin{align}
...
\end{align}
\dean{Need to say how the FT is used somewhere here before proceeding.}
...
The linear state-space representation of the MRAIDE allows the use of the well known Expectation Maximisation (EM) algorithm \cite{Dempster1977} to infer both the kernel and the field from electrophysiological data. The EM algorithm, when used in this context \cite{Dewar2009}, yields the maximum likelihood kernel estimate and the posterior distribution of the field over time. We use the standard EM algorithm for linear dynamic systems \cite{Shumway2000} and hence only describe aspects of the algorithm that are specific to MRA neural field equations. EM essentially finds increasingly tighter lower bounds on the likelihood of the kernel $p(\mathbf{y}_1 \ldots \mathbf{y}_T|w)$ so that, at convergence, the maximum of the bound corresponds to the maximum of the likelihood. 
