...
\author{In no particular order: \parham{P. Aram}, \mike{M. Dewar}, \dean{D.R. Freestone,~\IEEEmembership{Graduate Student Member,~IEEE,}}, \ken{K.Scerri}, D.B. Grayden,~\IEEEmembership{Member,~IEEE,} and V. Kadirkamanathan~\IEEEmembership{Member,~IEEE} % <-this % stops a space
...
\end{align}
\parham{where $\beta$ is constant with respect to $\boldsymbol\theta$ and }
\begin{align}\label{eq:upsilon0}
 \boldsymbol\upsilon_0 & =\sum_{i,j=1}^{n_x}[\boldsymbol\Xi_0]_{i,j}[\Sigma_{w}^{-1}\boldsymbol\Lambda_{x}^{-1}\odot\mathbf U]_{j,i}\\ 
 \boldsymbol\upsilon_1 & =\xi\sum_{i,j=1}^{n_x}[\boldsymbol\Xi_1]_{i,j}[\Sigma_{w}^{-1}\boldsymbol\Lambda_{x}^{-1}\odot\mathbf U]_{j,i}\label{eq:upsilon1} \\
...
\end{align}
\parham{where $\odot$ denotes block-wise matrix multiplication} and  $[.]_{i,j}$ denotes $\left(i,j\right)^{th}$ (block) element of the (block) matrix. Each $1\times n_{\theta}$ block of the $n_x \times n_x$ block matrix,  $\mathbf U$ is given by
\begin{align}
...
\begin{figure}[!h] 
 ...
 \caption{Actual (black line) and estimated (gray line) neural fields at a time instant for different spatial resolutions. (a) $j=0$, $n_x=17$, RMSE =4.44 . (b) $j=1$, $n_x=33$, RMSE =1.87 . (c) $j=2$, $n_x=65$, RMSE =1.2 . (d) $j=3$, $n_x=131$, RMSE = 1.04 . (e) $j=4$, $n_x=263$, RMSE = 1.04 .}
