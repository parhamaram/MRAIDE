...
\end{align}
and $[.]_{p,q}$ denotes $\left(p,q\right)$-element of the matrix, and $ [.]^{p,q}$ denotes $\left(p,q\right)$-block of the block matrix and where
\begin{align}
...
\end{align}
Each $1\times n_{\theta}$ block of $n_x \times n_x$ block matrix $\mathbf U$ is given by
\begin{align}
...
\end{align}
where $[.]_{p:} $ denotes $p^{th}$ row of the matrix. All $n_x^2$ blocks of $\Gamma_1$,$\Gamma_2$ and $\Gamma_3$  can be computed as a once-off before the commencement of the EM iterations. Note that to separate $\boldsymbol\theta$ from $\boldsymbol\Lambda_{\theta}$ in forming the $\mathcal{Q}$-function we used the relation
\begin{equation}
