...
The spatial cutoff frequency of the observed field was used to specify the level of decomposition required to represent the field using the wavelets. Fig.~\ref{fig:KernelAndFreqResponse} (b), shows the field spatial frequency and cuttoff of \dean{??}. Therefore, wavelets up to level $j=2$ (with the bandwidth $\approx[2.5,4]$ cycles/mm) can represent the significant spatial characteristics of the field. \cut{Note that wavelets due to their bandpass filter characteristics extract successively higher and higher frequency components by increasing $j$.}
