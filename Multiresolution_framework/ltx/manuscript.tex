...
\begin{IEEEkeywords}
Neural field model, Expectation Maximization (EM) algorithm, Multiresolution Approximation (MRA), wavelets.
\end{IEEEkeywords}
...
\end{equation}
where the post-synaptic membrane voltage at time $t$ of a population of neurons at position $\mathbf r$ is denoted $v_t\left(\mathbf r\right)$. Synaptic dynamics are included through the parameter $\xi=\left(1-\frac{ T_s}{\tau}\right)$, where $\tau$ is the synaptic time constant and $T_s$ is the sampling time. The connectivity strength between neural populations at a distance $\mid\mathbf{r}-\mathbf{r'}\mid$ is described by the kernel $w\left(\mathbf{r}-\mathbf{r}'\right)$. The connectivity kernel is taken as a ``Mexican hat'' function, which describes local excitation and lateral inhibition \cite{Amari1977}. The firing rate of the presynaptic neurons is related to the post-synaptic membrane potential by the activation function $f\left(.\right)$. The term $e_t(\mathbf r)$ is an $i.i.d.$ disturbance such that $e_t(\mathbf{s})\sim\mathcal{GP}(\mathbf 0,\eta(\mathbf{r}-\mathbf{r'}))$, where $\mathcal{GP}(\mathbf 0,\eta(\mathbf{r}-\mathbf{r'}))$  denotes a zero mean spatial Gaussian process with covariance function $\eta(\mathbf{r}-\mathbf{r'})$~\cite{Rasmussen2005}. For simplicity, we denote the current time step by $t$ and the future time step by $t+1$. The observation equation is given by 
...
The multiresolution approximation (MRA) of the neural IDE is be obtained by decomposing both the field, $v_t(.)$, and the connectivity kernel, $w(.)$, (assuming square-integrable functions) using translations and dilations of a scaling function $\phi(\mathbf{r})$ and a mother wavelet $\psi(\mathbf{r})$. Considering a one-dimensional field, the connectivity kernel is decomposed by,
\begin{equation}
...
% \end{equation}
Eqs \eqref{eq:KernelExpansion} and \eqref{eq:FieldExpansion} are infinite series expansions, but  for a practical implementation they must be truncated at some level $j$. Therefore, we have
 \begin{equation}
...
\end{equation}
where the unknown parameter $ \boldsymbol\theta \in \mathbb{R}^{n_{\theta}}$ and the unknown state  $ \mathbf{x}_t \in \mathbb{R}^{n_x}$ are defined as 
\begin{equation}
...
\label{FieldWeights}
\end{equation}
and where the kernel approximation coefficient vector $\boldsymbol \alpha_{j_0}$  and the field approximation coefficient vector $\mathbf{x}_{t,j_{0}}$ comprise all the coefficients $\left\lbrace\alpha_{j_0, l}:l \in \mathbb{Z} \right\rbrace $ and $\left\lbrace x_{t,j_0, l}: l \in \mathbb{Z}\right\rbrace$ respectively. Similarly the kernel and the field detail coefficient vectors, $\boldsymbol\beta_{j}$ and  $\check{\mathbf{x}}_{t,j}$ contain all the coefficient $\left\lbrace \beta_{j,l} :l \in \mathbb{Z}\right\rbrace$ and $\left\lbrace  \check x_{t,j, l}:l \in \mathbb{Z}\right\rbrace$ accordingly.
 The vector of the kernel scaling and wavelet functions $\boldsymbol\lambda$ and the vector of the field scaling and wavelet functions $\boldsymbol\mu$ are defined by
\begin{align}
...
\begin{equation}\label{eq:Lambdatheta}
 \mathbf{\Lambda}_{\theta}\triangleq\int_{\Omega}\boldsymbol\mu\left(\mathbf{r}\right) \int_\Omega { 
	...
	\, d\mathbf{r}'d\mathbf{r}}
\end{equation}
and substituting these into \eqref{eq:DecomposedModel2} and then cross-multiplying by $\mathbf{\Lambda}_{x}^{-1}$ yields
...
\end{equation}
and where $\mathbf I$ is the identity matrix, and 
...
\end{cases}
\end{equation}
In order to compute the  integrals (\ref{eq:Lambdax}) and (\ref{eq:Lambdatheta}) we need to be able to calculate the convolution and inner product of B-spline scaling functions. Equation (\ref{SplineConvolutionIntegral}) can be rewritten as $(m-1)$ times convolution of indicator function with itself 
\begin{equation}
...
By exploiting properties (\ref{eq:BsplineConvolution}) and (\ref{eq:BsplineInnerProduct}) the  integrals (\ref{eq:Lambdax}) and (\ref{eq:Lambdatheta}) can be computed. In order to calculate elements of $\boldsymbol\Lambda_{x}$ and $\boldsymbol\Lambda_{\theta}$, scaling and wavelet basis functions should be expanded  in terms of $N_m$ at the appropriate scale. 
In this work, 4th order cardinal B-spline scaling and wavelet functions are used therefore, to compute the integrals (\ref{eq:Lambdax}) and (\ref{eq:Lambdatheta})  the $8$th and $12$th order B-spline values at integer points are  required and can be found in \cite{Goswami1999}.
\section{State and Parameter Estimation}
A state-space representation of the IDE model allows the use of the Expectation Maximisation (EM) algorithm \cite{Dempster1977,Shumway2000} to infer both the kernel and the field from the electrophysiological data. The EM algorithm is a two part iterative algorithm that yields the maximum likelihood estimate of the connectivity kernel parameters and the posterior distribution of the field weights. In the M-step a lower bound $\mathcal{Q}\left(\boldsymbol\theta,\boldsymbol\theta'\right)$ of the complete-data log-likelihood  $p\left(\mathbf{X},\mathbf{Y} \mid \boldsymbol\theta \right)$, is maximised under the assumption of the system parameters being $\boldsymbol\theta'$. In the E-step, given an estimate of $\boldsymbol\theta$, the Rauch Tung Striebel (RTS) smoother \cite{Gibsona2005} can be employed to determine the posterior state sequence. 
The state-space representation of the IDE defined by \eqref{eq:StateEquation} and \eqref{eq:ReducedObservationEquation} has a quadratic $\mathcal Q\left(\boldsymbol\theta,\boldsymbol \theta'\right)$ in a form of
\begin{equation}
...
\end{align}
and $\beta$ is constant with respect to the connectivity kernel parameters. The $\left(i,j\right)^{th}$ element of the  block matrix $\mathbf U_{n_x \times n_x}$ is given by
\begin{align}
\left[ \mathbf U\right] _{i,j}&=\int_{\boldsymbol \Omega}[\mu(\mathbf r)]_i[\int_{\boldsymbol\Omega}\boldsymbol\mu\left(\mathbf{r'}\right)\boldsymbol\lambda^\top\left(\mathbf{r-r'}\right) d\mathbf{r'}]_{j:}d\mathbf r.
\end{align}
where $[.]_{j:} $ denotes $j^{th}$ row of the matrix. The second terms in equations (\ref{eq:upsilon0}-\ref{eq:Upsilon}) can be computed before EM implementation which increases the speed of the M-step significantly compared to the method in \cite{Dewar2009}. $\Xi$-variables can be calculated using the smoother outputs, i.e., state estimates, $\hat{\mathbf x}_t$ along with their covariance matrix, $\mathbf P_t$ and the cross-covariance matrix $\mathbf M_t$ using \cite{Gibsona2005}
...
One second of data was generated using equations \eqref{eq:DiscreteTimeModel} and \eqref{eq:ObservationEquation} in response to an initial condition where $\boldsymbol\Sigma_v=0.1 \times \mathbf{I}_{n_y}$ and $ \eta(r-r')=1\times\phi_{4,3,-2}(r-r')$. Sampling time and the synaptic time constant are set to 1 and 10 milliseconds respectively. The sensors were arranged regularly where the distance between adjacent sensors was set to 0.5 mm resulting into 161 observation locations. Sensors are modelled using $\phi_{4,4,k}$, providing a width of 0.8 mm at half the maximum amplitude of the observation kernel, allowing for some cross talk in the recordings. The connectivity kernel and its decomposition are plotted in  \figurename{\ref{fig:KernelAndFreqResponse}} (a). The spatial frequency of the observed field is shown in \figurename{\ref{fig:KernelAndFreqResponse}} (b). The observed field has an approximate spatial bandwidth $1.38$ cycles/mm. This suggests the use of wavelets up to level $j=3$. Field reconstruction at different level of approximations is shown in \figurename{\ref{fig:FieldEstimates}}. The figure confirms that $j=3$ is adequate to capture the dynamic of the underlying field. Root mean square error (RMSE) of the field estimates is shown in \figurename{\ref{fig:RMSE}}. The histogram of the parameter estimates in \figurename{\ref{fig:ParametersHistogram}} is generated using 100 runs of the EM algorithm, although the estimates of $\theta_1$ is biased (7.4 \%) the RMSE between the true and estimated kernels is $1.83$.
\begin{figure}[!h]
 \centering
