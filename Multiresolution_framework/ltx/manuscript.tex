...
\begin{equation}\label{eq:ApproxDiscreteTimeModel}
	...
	+ e_t\left(\mathbf{r}\right).
\end{equation}
The aim of the next part of the derivation is to isolate the state vector on the left hand side of the equation. To do this we first multiply both sides of equation~\eqref{eq:ApproxDiscreteTimeModel} by $\boldsymbol\mu\left(r\right)$ and integrate over space giving (\dean{UP TO HERE})
\begin{equation}\label{eq:ApproxDiscreteTimeModel2}
	\int_{\Omega} \boldsymbol\mu\left(r\right)\boldsymbol\mu^\top\left(\mathbf{r}\right) \mathrm{d}\mathbf{r} \mathbf{x}_t = 
	\xi \int_{\Omega}\boldsymbol\mu\left(r\right)\boldsymbol\mu^\top\left(r\right) \mathrm{d}\mathbf{r} \mathbf{x}_t + 
	T_s \int_{\Omega}\boldsymbol\mu\left(r\right)\int_\Omega { 
	    \boldsymbol\theta^\top\boldsymbol\lambda\left(\mathbf{r}-\mathbf{r}'\right)
	    f\left(\boldsymbol\mu^\top\left(\mathbf{r}'\right)\mathbf{x}_t\right) 
	\, \mathrm{d}\mathbf{r}'\mathrm{d}\mathbf{r}}  
	+ \int_{\Omega}\boldsymbol\mu\left(r\right)e_t\left(\mathbf{r}\right)\mathrm{d}\mathbf{r}.
\end{equation}

To derive the state-space model, we first define the matrices 
\begin{align}\label{eq:Lambdax}
 \mathbf{\Lambda}_{x} &\triangleq \int_{\Omega}\boldsymbol\mu\left(r\right)\boldsymbol\mu^\top\left(r\right) \mathrm{d}\mathbf{r} \\
