...
\caption{ {\bf The IDE neural field model components}. (\textbf{A}) The Laplacian (dotted line) and Gaussian (solid line) functions which are typically used in constructing Mexican hat and  Wizard hat connectivity kernels. (\textbf{B}) First order (dotted line) and second order (solid line) synaptic kernels. (\textbf{C}) A representative of the activation function.}
