...
\hline \hline
\end{tabular}}
 \caption {{\bf The Computational complexity of the estimation algorithm}. A comparison between the algorithm proposed herein and that of \citet{Dewar2009} is also provided. The dashes indicate equal complexities for both algorithms.} 
...
\multicolumn{1}{|p{16cm}|}{Choose a bounded random state vector sequence and set $\mathbf P_t$ and $\mathbf M_t$ large} \\
...
\multicolumn{1}{|p{16cm}|}{Compute all elements of $\boldsymbol\Lambda_{\theta}$ using} \\
$\left[ \boldsymbol\Lambda_{\theta}\right] _{k,i}=\left[ \mathbf U\right]^{k,i}\boldsymbol\theta$\\
