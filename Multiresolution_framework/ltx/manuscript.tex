...
Alternative basis functions can be used to reduce the neural field formulations, for example Gaussian basis functions are adopted by \cite{Freestone2011}. One clear advantage of B-spline scaling and wavelet functions is the MRA capability, allowing the neural field reconstruction at different spatial scales. B-spline scaling and wavelet functions are compactly supported, eliminating  numerical errors caused by the Gaussian basis function. The partition of unity of these basis functions enable the reconstruction of the neural field when it saturates, this may be beneficial to model the neurological disorder such as seizure.    }
