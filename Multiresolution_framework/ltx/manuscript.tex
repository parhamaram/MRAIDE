...
\begin{equation}
 ...
 and the posterior distribution of the field over time. Here $p(\mathbf Y;\boldsymbol\theta)$ denotes the probability density function of the entire sequence of observations $\mathbf Y$ parameterised  by $\boldsymbol\theta$ known as the likelihood function. When maximisation of the likelihood function is difficult to achieve directly the EM algorithm can be used to find increasingly tighter lower bounds on the likelihood of the parameters so that, at convergence, the maximum of the bound corresponds to the maximum of the likelihood. By exploiting the total probability rule it is rather straightforward to show that 
\begin{equation}
...
\end{equation}
is a lower bound on the log-likelihood function \cite{Bishop2006} where $p(\mathbf X,\mathbf Y;\boldsymbol \Theta)$ is the joint probabilty distribution of the states and the observations. Maximising the likelihood in terms of the parameter set is equivalent to the log-likelihood maximisation, as both the likelihood and log-likelihood functions share the same maximiser due to the monotonicity of the logarithm function.... The rest of this section is to construct the lower bound $\mathcal Q$ for the linear state-space model given by \eqref{eq:StateEquation} and \eqref{eq:ReducedObservationEquation}. The joint probability distribution $p(\mathbf X,\mathbf Y;\boldsymbol \Theta)$ can be written as
