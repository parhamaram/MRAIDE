...
\end{equation} 
\dean{Can we define something like the other paper like
\begin{equation}
	\left[\Gamma(\mathbf{r}')\right]_{:i} = T_s \mathbf{\Lambda}_{x}^{-1}\int_{\Omega} \boldsymbol\mu\left(\mathbf{r}\right)\boldsymbol\lambda\left(\mathbf{r}-\mathbf{r}'\right) \,\mathrm{d}\mathbf{r}
\end{equation}
giving
\begin{equation}
	\mathbf{x}_{t+1} = 
