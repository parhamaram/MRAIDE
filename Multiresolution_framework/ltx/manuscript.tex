...
\end{align}
where $[.]_{i,j}$ denotes $\left(i,j\right)$-element of the matrix, and $ [.]^{i,j}$ denotes $\left(i,j\right)$-block of the  block matrix and where
\begin{align}
...
\end{align}
where $[.]_{j:} $ denotes $j^{th}$ row of the matrix. All blocks of $\Gamma_1$,$\Gamma_2$ and $\Gamma_3$  can be computed as a once-off before the commencement of the EM iterations. Note that to separate $\boldsymbol\theta$ from $\boldsymbol\Lambda_{\theta}$ in forming the $\mathcal{Q}$-function we used the relation
\begin{equation}
