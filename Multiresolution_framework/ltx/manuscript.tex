...
\end{align}
where $[\cdot]_{i:} $ denotes the $i^{th}$ row.  Each element of $\boldsymbol\Lambda_{\theta}$ can be calculated using
...
\end{equation}
The matrices $\boldsymbol\Xi_0$ and $\boldsymbol\Xi_1$ in Eqs.~\eqref{eq:epsilon0}-\eqref{eq:Epsilon} are calculated using the RTS smoother outputs: $\hat{\mathbf x}_t$, covariance, $\mathbf P_t^b$, and cross-covariance matrix, $\mathbf M_t$ \citep{Gibsona2005}, 
\begin{align}
...
\section{Lower bound}\label{ap:QDerivation}
...
\end{equation}
where
\begin{equation}\label{eq:appepsilon0}
\boldsymbol\upsilon_0=\sum_{i,j=1}^{n_x}[\boldsymbol\Xi_0]_{i,j}[\boldsymbol\Gamma_1]_{j,i},
\end{equation}
\begin{equation}\label{eq:appepsilon1}
\boldsymbol\upsilon_1=\xi\sum_{i,j=1}^{n_x}[\boldsymbol\Xi_1]_{i,j}[\boldsymbol\Gamma_1]^{j,i},
\end{equation}                                                                                 
and
\begin{equation}\label{eq:appEpsilon}
\boldsymbol\Upsilon=T_s\varsigma\sum_{i,j=1}^{n_x}[\boldsymbol\Xi_1]_{i,j}[\boldsymbol\Gamma_2]^{j,i}.
\end{equation}
